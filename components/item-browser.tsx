...
export function ItemBrowser({ items }: { items: WikiItem[] }) {
  ...
  return <section className="item-browser" aria-label="Items database">
    ...
    <p className="results-count" role="status" aria-live="polite">Showing {filtered.length} / {items.length} entries</p>
    {filtered.length > 0 ? <div className="item-grid">{filtered.map((item) => <ItemCard key={item.slug} item={item} />)}</div> : <p className="empty-copy">No items match this search or category.</p>}
  </section>;
}
